...
kubectl exec mongo-0 -- mongo --eval 'rs.initiate({ _id : "rs0", members: [ { _id : 0, host : "mongo-0.mongo:27017" }, { _id : 1, host : "mongo-1.mongo:27017" }, { _id : 2, host : "mongo-2.mongo:27017" }]});'
...
kubectl exec mongo-0 -- mongo --eval 'db.getSiblingDB("admin").createUser({user:"admin",pwd:"schdsvsbvj",roles:[{role:"root",db:"admin"}]});'
echo
